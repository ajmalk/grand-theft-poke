...
public class WalletTest extends junit.framework.TestCase {
	...
	/**
	 * Checks that when you update the price for a sale, that it updates to the
	 * correct price
	 */
	@Test
	public void testUpdateMoney() {
		...
		price = ((MarketPlaceItem) myMarket.getStock().get(
				items.getStrengthRoid())).getPrice();
	...
	public void testCheckAmount() {
		...
		float money;
		boolean test;
		price = ((MarketPlaceItem) myMarket.getStock()
				.get(items.getTradeRoid())).getPrice();
